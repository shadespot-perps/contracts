 ...
 *
 * Flow: Open with plain ERC-20 collateral → Close with encrypted token payout
 *
 * Prerequisites:
 ...
 *   6. requestCloseEncryptedPayout: user flags position for encrypted token payout
 *   7. finalizeCloseEncryptedPayout: keeper settles PnL, mints encrypted tokens to trader
 *
 * Encryption check: even though collateral entered as plain ERC-20, it is immediately
 * wrapped to euint64 on-chain — position is fully encrypted inside PositionManager.
 */
...
async function main() {
  ...
  console.log("Flow:   plain-open → encrypted-close\n");
  ...
  console.log("\n[Close — user requests encrypted token payout]");
  const reqTx = await fheRouter.requestCloseEncryptedPayout(positionId);
  await reqTx.wait();
  console.log("requestCloseEncryptedPayout tx:", reqTx.hash);

  // Keeper finalizes — same wallet on testnet.
  // On real CoFHE: finalAmount/sizePlain/collateralPlain come from the dispatcher.
  console.log("\n[Close — keeper finalizes encrypted token payout]");
  const finTx = await fheRouter.finalizeCloseEncryptedPayout(
    positionId,
  ...
  console.log("finalizeCloseEncryptedPayout tx:", finTx.hash);

  const iface = new ethers.Interface(FHE_ROUTER_ABI);
  for (const log of finReceipt?.logs ?? []) {
    try {
      const parsed = iface.parseLog({ topics: [...log.topics], data: log.data });
      if (parsed?.name === "EncryptedPayoutSettled") {
        console.log("\nEncryptedPayoutSettled (encryption verified):");
        console.log("  trader:", parsed.args.trader);
        console.log("  amount:", parsed.args.amount.toString(), "(encrypted units)");
        console.log("  Payout minted as encrypted tokens — balance stays private.");
      }
    } catch {}
  }
}
